	...
	template: 
        '<div class="functional-select-wrapper" v-on:click.stop="multipleFocus()">' +
            '<label class="display-container multiple-select-container clearfix" v-bind:class="(show)?\'single-selected-focus\':\'\'">' +
                ...
                '</p>' +
                '<p class="multiple-selected-item" v-for="item in selectedList" v-bind:key="item.index">{{ item.name }}<i v-on:click.stop.prevent="multipleRemove(item.id)">×</i></p>' +
                '<i class="drop">▼</i>' +
            '</label>' +
            '<div class="options-container" v-show="show">' +
                '<div class="search-container">' +
                    '<input placeholder="请输入" class="search-input" v-model="search" v-on:keyup="multipleSearch($event)" v-on:click.stop />' +
                '</div>' +
